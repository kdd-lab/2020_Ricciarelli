...
for year in years:
    ...
    with gzip.open(path_to_nets + str(year) + '.gz', 'r') as edge_list:
        for record in tqdm(edge_list, desc='YEAR {}: BUILDING NETWORK'
            ...
            g.add_edge(params[0], params[1], weight=int(params[2]))
    ...
    if sys.argv[3] == 'Y':
        ...
        degree_centrality = sorted([v for k, v in
        ...
        axs.text(0.65, 0.75, 'mean = {}\nmax = {}\nmin = {}'
                 .format(np.format_float_scientific(np.mean(degree_centrality),
                         2),
                         np.format_float_scientific(max(degree_centrality), 2),
                         np.format_float_scientific(min(degree_centrality),
                                                    2)),
                 bbox=dict(facecolor='white', edgecolor='black'),
        ...
        fig.savefig('../datasets/weighted_coauthors_networks/{}/'
                    'degree_centrality'
                    '_distribution_{}.pdf'.format(years[0], year),
                    format='pdf')
        plt.close(fig=fig)
        ...
        axs.grid(axis='y', linestyle='--', color='black', zorder=1)
        axs.text(0.65, 0.75, 'mean = {}\nmax = {}\nmin = {}'
                 .format(np.format_float_scientific(
                                                    np.mean(
                                                        closeness_centrality),
                                                    2),
                         np.format_float_scientific(
                                                    max(closeness_centrality),
                                                    2),
                         np.format_float_scientific(
        ...
        axs.text(0.65, 0.75, 'mean = {}\nmax = {}\nmin = {}'
                 .format(
                    np.format_float_scientific(np.mean(betweenness_centrality),
                                               2),
                    np.format_float_scientific(max(betweenness_centrality),
                                               2),
